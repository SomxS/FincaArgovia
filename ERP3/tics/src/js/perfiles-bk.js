...
$(function () {
    ...
    $("#form__permisos").on("submit", (e) => {
        ...
        if (!IPT.val()) {
            ...
            IPT.next("span").html('<i class="icon-warning-1"></i> Campo requerido.');
        } else if (listPerfiles.some((p) => p.perfil.toUpperCase() === IPT.val().toUpperCase())) {
            IPT.focus();
            IPT.addClass("is-invalid");
            IPT.next("span").removeClass("hide");
            IPT.next("span").html(`
            ...
            const VALOR = IPT.val().charAt(0).toUpperCase() + IPT.val().slice(1);
...
function tablePerfiles() {
    ...
    send_ajax(datos, ctrlPerfil, "#tbDatosPerfiles").then((data) => {
        listPerfiles = data;
        let tbody = "";
        data.forEach((p) => {
            const ID = p.id;
            ...
            <tr>
                ...
                <td class="text-center">
                    <button type="button" title="Editar" class="btn btn-outline-primary" onClick="modalEditPerfil(${ID}, '${PERFIL}') "><i class="icon-pencil"></i></button>
...
function estadoPerfil(id, perfil) {
    swal_question(`¿REALMENTE DESEA DESACTIVAR EL PERFIL "${perfil.toUpperCase()}"?`).then((result) => {
...
function modalEditPerfil(id, perfil) {
    perfilesList = "";
    ...
    modal = bootbox.dialog({
        ...
        message: `
                ...
                <div class="col-12 mb-3 d-flex justify-content-between">
                    <button  class="btn btn-primary col-5" onclick="editarPerfil(${id});">Actualizar</button>
...
function editarPerfil(id) {
    ...
    if (!IPT.val()) {
        ...
        IPT.addClass("is-invalid");
        IPT.next("span").removeClass("hide");
        IPT.next("span").html('<i class="icon-warning-1"></i> Este perfil ya existe, intente con otro nombre.');
    } else {
        let datos = new FormData();
...
function tbModulos(id, perfil) {
    ...
    send_ajax(datos, ctrlPerfil, DIVTB).then((data) => {
        let tbody = "";
        data.forEach((m) => {
            const IDM = m.id;
        ...
        <table class="table rounded-4 overflow-hidden" id="tbDirectory">
            <thead>
                <tr>
                    <th class="bg-primary" colspan="5">${perfil}</th>
                </tr>
            </thead>
            <tbody>
                ${tbody}
...
function checkAllDirectory(idS, idM, perfil) {
    ...
    swal_question('¿Esta seguro de continuar?', "Esto afectara todos los directorios.").then((result) => {
        if (result.isConfirmed) {
